...
class BedEntry(object):
    ...
    def __repr__(self):
        return 'BedEntry(%s, %i, %i)' % (repr(self.refseq), self.position, self.position + self.length)

    def __str__(self):
        print('Bed Entry chr=%s, start=%i, length=%i, strand=%s, score=%s, name=%s' % (
            self.refseq, self.position, self.length, self.strand, self.score, self.name))
...
def write_bed(filenameOrFileObject, reads, name_to_chromosome_lookup_or_none,
              track_name, include_header=True, minimal=False):
...
def bed_to_bigbed(input_bed_filename, output_filename, chromosome_lengths, already_sorted=False):
            ...
            if line and not line.startswith(b'#') and not line.startswith(b'track'):
                ...
                try:
                    ...
                    try:
                        ...
                        if strand is None:
                            strand = '.'
                    out.append((chr, min(start, stop), max(
                        start, stop), name, 0,  strand))
                except KeyError:
                    missing_chroms.add(chr)
        if missing_chroms:
            print('could not find the following chromosomes %s' %
                  (missing_chroms, ))
        out.sort()
        inputtf = tempfile.NamedTemporaryFile()
        for chr, start, stop, name, score, strand in out:
            inputtf.write(
                ("%s\t%s\t%s\t%s\t%s\t%s\n" % (chr, start, stop, name, score, strand)).encode('utf-8'))
        inputtf.flush()
        inputtf.seek(0, 0)
        input.close()
    else:
    ...
    cmd = [os.path.join(bedToBigBedPath, 'bedToBigBed'), '-tab',  in_name, os.path.abspath(
        chrom_sizes_file.name), os.path.abspath(output_filename), '-type=bed6']
    print(cmd)
...
def write_big_bed(input_dataframe_or_list_of_bed_entries, output_filename, chromosome_lengths):
